...
internal static class Opl3Tables
{
    ...
    private static ushort[] BuildWaveformData()
    {
        ...
        {
            var quarterWave = (phase & 0x100) != 0 ? LogSinRomData[(phase & 0xff) ^ 0xff] : LogSinRomData[phase & 0xff];
            var sign = (ushort)((phase & 0x200) != 0 ? 0x8000 : 0);
            var doubledWave = sign != 0 ? (ushort)0x1000 :
                (phase & 0x80) != 0 ? LogSinRomData[((phase ^ 0xff) << 1) & 0xff] : LogSinRomData[(phase << 1) & 0xff];
